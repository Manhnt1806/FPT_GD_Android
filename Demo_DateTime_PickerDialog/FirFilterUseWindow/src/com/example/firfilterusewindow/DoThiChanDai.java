...
public class DoThiChanDai extends Activity{
	GraphicalView windowChart;
	GraphicalView filterChart;
	GraphicalView magnitudeChart;
	LinearLayout cuasochandai;
	LinearLayout dapungxungchandai;
	LinearLayout dapungbiendochandai;
	Button btWindow;
	Button btChart;
	Button btmagnitude;
	float window[];
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.dothichandai);
		cuasochandai=(LinearLayout)findViewById(R.id.cuasochandai);
		dapungxungchandai=(LinearLayout)findViewById(R.id.dapungxungchandai);
		dapungbiendochandai=(LinearLayout)findViewById(R.id.dapungbiendochandai);
		btCheck=(Button)findViewById(R.id.btCheckChanDai);
		...
		//lấy intent gọi Activity này
		 ...
		 callerIntent.getBundleExtra("chandaiPackage");
		 ...
		 cuasochandai.addView(windowChart);
		 dapungxungchandai.addView(filterChart);
		 dapungbiendochandai.addView(magnitudeChart);
		 btCheck.setOnClickListener(new View.OnClickListener() {
				
				@Override
				public void onClick(View arg0) {
					// TODO Auto-generated method stub
					Intent myIntent=new Intent(DoThiChanDai.this,CheckActivity.class);
					if(N!=0){
	...
	public void createChart(){
		float anpha=N/2;
		BoLocChanDai.getInstance().setDoRongGiaiChuyenTiep((float) (deltaW*Math.PI));
		BoLocChanDai.getInstance().setSuyHaoDaiChan(As);
		BoLocChanDai.getInstance().setTanSoThongTren(tansoTren);
		BoLocChanDai.getInstance().setTanSoThongDuoi(tansoDuoi);
		
		for(int i=0;i<=N;i++){
			chart[i]=BoLocChanDai.getInstance().gtBoLoc(i-anpha)*window[i];
		}
